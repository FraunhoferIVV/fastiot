...
def new_project(project_name: str = typer.Argument(None, help="The project name to generate"),
    ...
    if context.project_namespace != "" and not force:
        logging.error("An existing project configuration has been found with project namespace %s. "
                      ...
                      context.project_namespace)
        raise typer.Exit(2)

    if target_dir.startswith('/'):
        context.project_root_dir = target_dir
    else:
        context.project_root_dir = os.path.join(context.project_root_dir, target_dir)
    ...
    with open(os.path.join(context.project_root_dir, 'configure.py'), "w") as docker_compose_file:
