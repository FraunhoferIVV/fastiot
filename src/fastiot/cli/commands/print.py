...
class ToPrint(str, Enum):
    ...
    service_package_names = 'service_package_names'
    service_to_build = 'service_to_build'
    npm_test_dir = 'npm_test_dir'
    test_config = 'test_config'
    test_package = 'test_package'
    infrastructure_services = 'external_services'
    infrastructure_services_port_vars = 'external_services_port_vars'
...
printed.\n
* services_package_names ... prints all packages which include regular services.\n
* services_to_build ... prints all services which can be built.\n
...
def print_cmd(name: str = typer.Argument(..., help=_name_help, callback=check_name, shell_complete=_name_completion),
              config: Optional[str] = typer.Option(None,
                                                   help="Only applicable if <name> is services_to_build. If specified, "
                                                        "it doesn't print services which are not needed for the "
                                                        "configuration."),
              platform: Optional[str] = typer.Option(None,
                                                     help="Only applicable if <name> is services_to_build. If "
    ...
    elif name == 'service_package_names':
        for package in project_config.service_packages:
            print(package.package_name)
    elif name == 'services_to_build':
        if config is not None:
            deployment = project_config.get_deployment_by_name(config)
            print("\n".join(deployment.fastiot_services))
        else:
            for service in project_config.get_all_services():
                if len(platforms) > 0:
    ...
    elif name == 'infrastructure_services':
        print("\n".join(list(get_services_list().keys())))
    elif name == 'infrastructure_services_port_vars':
